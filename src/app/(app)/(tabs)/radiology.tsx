...
import { RadiologyCard } from "@/src/features/radiology/radiology-card";
...
export default function Radiology() {
  ...
  const { data, isLoading, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    useRadiology({ name: debouncedSearch, governorateId: filters.governorateId, cityId: filters.cityId });
    ...
    <ScreenWrapper isLoading={isLoading} isScrollable={false} bottomPadding={0}>
      <View className="flex-1 px-5">
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <RadiologyCard radiology={item} onPress={() => router.push(`/(app)/radiology/${item.id}` as any)} />
          )}
