...
import { LabCard } from "@/src/features/laboratories/lab-card";
...
export default function Labs() {
  ...
  const { data, isLoading, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    useLabs({ name: debouncedSearch, governorateId: filters.governorateId, cityId: filters.cityId });
    ...
    <ScreenWrapper isLoading={isLoading} isScrollable={false} bottomPadding={0}>
      <View className="flex-1 px-5">
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <LabCard lab={item} onPress={() => router.push(`/(app)/lab/${item.id}` as any)} />
          )}
