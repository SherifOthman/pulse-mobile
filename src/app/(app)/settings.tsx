...
import { Button, ListGroup, Separator, Switch, Text, useThemeColor } from "heroui-native";
...
export default function Settings() {
  ...
  return (
    <View className="flex-1 bg-background">
      <View className="flex-row-reverse items-center px-5 pb-4 border-b border-border" style={{ paddingTop: insets.top + 12 }}>
        <Text.Heading type="h4" weight="bold">الإعدادات</Text.Heading>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, gap: 24 }} showsVerticalScrollIndicator={false}>
        <View className="gap-2">
          <Text.Paragraph type="body-sm" color="muted" className="mr-1 text-right">المظهر</Text.Paragraph>
          <ListGroup>
            <ListGroup.Item>
              <ListGroup.ItemPrefix>
                <View className="w-9 h-9 rounded-full bg-accent/10 items-center justify-center">
                  <Ionicons name={isDark ? "moon" : "sunny-outline"} size={18} color={accent} />
                </View>
              </ListGroup.ItemPrefix>
              <ListGroup.ItemContent>
                <ListGroup.ItemTitle>الوضع الليلي</ListGroup.ItemTitle>
                <ListGroup.ItemDescription>{isDark ? "مفعّل" : "معطّل"}</ListGroup.ItemDescription>
              </ListGroup.ItemContent>
              <ListGroup.ItemSuffix>
                <Switch isSelected={isDark} onSelectedChange={toggleTheme} className="w-14 h-8">
                  <Switch.Thumb className="size-6" animation={{ left: { value: 4, springConfig: { damping: 30, stiffness: 300 } } }} />
                  <Switch.StartContent className="left-1.5">
                    {isDark && <Animated.View entering={ZoomIn.springify()}><Ionicons name="moon" size={14} color="white" /></Animated.View>}
                  </Switch.StartContent>
                  <Switch.EndContent className="right-1.5">
                    {!isDark && <Animated.View entering={ZoomIn.springify()}><Ionicons name="sunny" size={14} color="white" /></Animated.View>}
                  </Switch.EndContent>
                </Switch>
              </ListGroup.ItemSuffix>
            </ListGroup.Item>
        ...
        <View className="gap-2">
          <Text.Paragraph type="body-sm" color="muted" className="mr-1 text-right">الإشعارات</Text.Paragraph>
          <ListGroup>
            <ListGroup.Item>
              <ListGroup.ItemPrefix>
                <View className="w-9 h-9 rounded-full bg-accent/10 items-center justify-center">
        ...
        <View className="gap-2">
          <Text.Paragraph type="body-sm" color="muted" className="mr-1 text-right">حول التطبيق</Text.Paragraph>
          <ListGroup>
            <ListGroup.Item>
              <ListGroup.ItemPrefix>
                <View className="w-9 h-9 rounded-full bg-accent/10 items-center justify-center">
